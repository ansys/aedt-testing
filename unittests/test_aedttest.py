...
def test_allocate_task_multiple():
    ...
    allocated_machines = allocate_task({"cores": 16, "parametric_tasks": 2})
    assert allocated_machines == {"host1": {"cores": 16, "tasks": 2}}

    allocated_machines = allocate_task({"cores": 24, "parametric_tasks": 2})
    assert not allocated_machines

    allocated_machines = allocate_task({"cores": 10, "parametric_tasks": 2})
    assert allocated_machines == {"host1": {"cores": 10, "tasks": 2}}

    allocated_machines = allocate_task({"cores": 25, "parametric_tasks": 5})
    ...
    allocated_machines = allocate_task({"cores": 26, "parametric_tasks": 2})
    assert not allocated_machines
