...
def get_job_machines(custom_input=None):
    ...
    Args:
        custom_input: (str) if scheduler is not in supported list, then user needs to
                provide string with available hosts and cores. Format: "host1:15,host2:10"

    Returns:
        (tuple[hostinfo]) all available machines
...
def parse_hosts_lsf(host_list_str):
    ...
    Parse LSF hosts
    Args:
        host_list_str: format from env var host_nameA num_processors1 host_nameB num_processors2
...
def parse_hosts_ccs(host_list_str):
    ...
    Args:
        host_list_str: (str) format "#hosts host1 #cores1 host2 #cores2 host3 #cores3 ... hostN #coresN"
...
def _parse_single_host(unparsed_str):
    ...
    Parse host string
    Args:
        unparsed_str: (str) format 'host_c[008-010,012,017-019]'
